...
class ContractUtils {
  ...
  constructor(token, abi, signer, address, gasPrice, provider, hasDecimal = true) {
    ...
    if (token === '') {
      ...
      this.batchCall = new Contract(token, abi);
    }
    if (hasDecimal) {
      if (token === '') {
        this.decimal = 18
  ...
  //   // 授权711合约
  // export const approve711ContractMethod = async (nftContract: any, approveAddr: string, myAddress: string,callback: any) => {
  //   const [err2, allowce] = await toAsync(
  //     nftContract.isApprovedForAll(myAddress, approveAddr)
  //   );
  //   if(err2 == null) {
  //     if(!allowce){
  //       estimateContractMethod(nftContract, 'setApprovalForAll',[approveAddr, true], callback)
  //     } else {
  //       callback && callback()
  //     }
  //   } else {
  //     cacheResponseError(err2)
  //     return false
  //   }
  // }

  // 授权NFT合约
  async approveNftMethod(approveAddr, callback) {
    const params = [this.address, approveAddr]
    const allowanceRes = await this.method('isApprovedForAll', params)
    if (allowanceRes[0] == null) {
      if (!allowanceRes[1]) {
        const estRes = await this.estimateMethod('setApprovalForAll', () => {
          return callback()
        }, [approveAddr, true])
        return estRes
      } else {
